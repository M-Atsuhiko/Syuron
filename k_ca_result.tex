...
結果を示す. 図中の凡例や検定手法とその結果の示し方は図\ref{Ka_Result1}と同様である.
     ...
     \ref{k_ca_K_dist_dt30}に, CaTコンダクタンス分布を図\ref{k_ca_Ca_dist_dt30}に
     示す. 
     %% \begin{figure}[H]
     %%   \begin{subfigure}{0.5\columnwidth}
     %%     \centering
...
% 最後にそれぞれの分布パターン、コンダクタンス考慮の組で、4パターンのコンダクタンスによる結果を一つのグラフにまとめる
     ...
     に従って決定する場合, $\mu$, $\sigma$, $peak$の値を用いる. しかし$peak$が0に
     近い場合は$\mu$, $\sigma$の値は意味をなさない. そのためピーク位置の解析では
     最大コンダクタンス値の5\%以上の$peak$をとった場合の$mean$のみを用いた. 樹状突起の長さは
     全て[0, 1]に正規化されており, 1に近いほど細胞体から遠ざかる. また図\ref{Gaus_peaks}上部の星印($\star$)は
     ...
     \begin{figure}[H]
       ...
       \vspace{-1cm}
       \caption{ガウス分布を用いたKaコンダクタンスのピーク位置}
       \label{Gaus_peaks}
     \end{figure}
