...
\chapter{研究手法・原理}
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{神経細胞形態の表現}
     ...
     また細胞体と直接結合している$Branch$を特にStemと呼ぶ. 1本のStemと, 
     そのStemから伸びている$Branch$からなる樹状構造をあわせて1つの樹状突起(Dendrite)
 ...
 \subsection{記号Sの置き換え規則}
   ...
   \begin{figure}[H]
     \centering
     \includegraphics[width=5cm]{./Images/path_S.eps}
     \caption{${\rm path(S_i)}$}
     \label{path(S_i)}
   \end{figure}
   以下に記号$S_i$の置き換え規則($*$)の選択方法と, その際に考慮するヒュー
   リスティックスについて示す. ここで$N_B$を各置き換えステップにおいての
   記号$Bh$の総個数, $N_T$を記号$T$の総個数であるとする. また初期状態は$S_0$である. 
   \begin{enumerate}
...
%           $\overline{g}_{Ka}$ [${\rm S/cm^2}$]& 0.01\\ \hline
 ...
 \section{作成した神経細胞の数理モデル化}
   ...
   \subsection{コンパートメント上のコンダクタンス分布設定}
     ...
     を設定する. 
     樹状突起上のコンダクタンス分布は線形な分布とガウス分布の2つを仮定した. 線形なコンダクタンス分布は先行研究で用いられていた
     ...
     \item \textbf{線形分布の場合}\\
       ...
       上記の$diam$と同様に親$Branch$の実数倍として決定する\\
       \begin{align}
