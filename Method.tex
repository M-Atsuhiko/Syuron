...
\chapter{研究手法・原理}
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{パラメータセットからのStem, Branch生成}
     ...
     パラメータセットを持ち, そこから2本の樹状突起が生成される. パラメータセットに含まれるパラメータ
     を以下に示す. 表\ref{morpho_parameter}は樹状突起形態を生成するために使用する
     パラメータである. 
   ...
   座標情報を設定することができる. 原点から, ある記号$1_x$に至
   るまでに通過するStemやBranchの長さの和を${\rm path(1_x)}$とする. ここで記号
   ...
   \begin{enumerate}
     \item \textbf{Termination $ 1 \to 3$} \\
       ...
       \begin{equation}
	 {\rm Cumulative}\hspace{0.1cm}\gamma\hspace{0.05cm}({\rm path(1_x)}) > rand
       \end{equation}
   ...
   状突起として妥当な形態を得るために導入するヒューリスティックである.

   \begin{itemize}
    \item ${\rm path(1_x)} > 2000$のとき
    \item $diam_P <= Minimum diameter$ のとき\\
	  この場合は$diam_P$をMinimum diameterに設定し直す. 
   \end{itemize}
   ...
   \begin{itemize}
     \item \textbf{線形分布の場合}\\
       CaT, Kコンダクタンス値について, 上記のdiameterと同様の方法で決定する\\
      \begin{tabular}[c]{ll}
...
%
   ...
   \subsection{シナプティックゾーンとシナプスの付与}
     ...
     ンに入った場合, 樹状突起上の$5{\mu}m$ごとに1つのシナプスを付与する. % <- 5マイクロm ってどういう向きの？
     上下のシナプテックゾーン, シナプスが付与された樹状突起の例を図\ref{synapse}に示す.
     \begin{figure}[h]
       \centering
       \includegraphics[width=5cm]{./Images/temp.png} 
   ...
   \subsection{数理モデル}
     ...
     \begin{itemize}
       \item 電位依存型T型${\rm Ca^{2+}}$電流
         \begin{align}
           I_{CaT} &= g_{CaT} {\cdot} ghk(V_{i},cai,cao) \\
...
%
	...
	\begin{tabular}[t]{|c|c|} \hline
	 ...
	 $T$ [${\rm ^{\circ}C}$] & 37\\ \hline
         $\overline{g}_{CaT}$ [${\rm S/cm^2}$]&\\ \hline
         $\overline{g}_{Ka}$ [${\rm S/cm^2}$]&\\ \hline
         ...
         $gmn$ & 0.6\\ \hline
         $gml$ & 1\\ \hline
         $V_{\frac{1}{2}n}$ [mV]& -33.6\\ \hline
         $V_{\frac{1}{2}l}$ [mV]& -83\\ \hline
 ...
 \section{遺伝アルゴリズム}
   神経細胞を個体として200個体, 世代数500で遺伝アルゴリズムを用いたパラメー
   ...
   \subsection{個体評価}
     ...
     以下に個体の評価方法を述べる. \\
     まず, シミュレーションを行うことができない個体をどのように評価する
     かを以下に示す.
     \begin{itemize}
     \item \textbf{形態的エラー}\\
       以下の場合はシミュレーションを行わず, $\mu = -100$, $sigma = 5$の
       ガウス分布から乱数を発生させ, その値を評価値とした. 
       \begin{itemize}
         \item 神経細胞全体でStemとBranchの個数が合計15未満の場合
         \item 神経細胞全体で樹状突起の分岐が一つもない場合
	 \item 上下のシナプティックゾーンそれぞれにシナプスを作れなかった場合
       \end{itemize}
     \end{itemize}
     次にシミュレーションの結果に対して以下のヒューリスティックを導入す
     る. 
     \begin{itemize}
     \item \textbf{EPSPエラー}\\
	   シミュレーションを行った場合でも以下の場合は$\mu = -50$,
	   $\sigma = 5$のガウス分布から乱数を発生させ, その値を評価値とした.
       \begin{itemize}
         \item 一方のシナプティックゾーンのみでの発火による細胞体での膜
	       電位の脱分極が2mV以下の場合 % <- ここ書き直す
       \end{itemize}
       以下の項目は神経細胞の膜状にKaチャネル, CaTチャネルを分布させ
       る場合にのみ考慮する. これは神経細胞での発火や, 膜電位の振動を % <- 細胞体での発火を抑制する意味は? ここコンダクタンスの時のみの考慮にしなくてもよさそう
       抑制するためである.
       \begin{itemize}
