       ...
       \item \textbf{突然変異 Mutation}\\
        パラメータセットの各項目に対して0.65の確率でガウス分布G($\mu$,${{\sigma}_M}^2$)に従う変動を与える. すなわち, あるパラメータ値
	     ...
	     \begin{tabular}[t]{|l||c|c|c|c|} \hline
	       ...
	       Ka \textit{Stem conductance}	  & 0.001	      & [0, 0.22] & U(0,0.08)\\ \hline
	       Ka \textit{peak}			  & 0.001     	      & [0, 0.22] & U(0,0.22)\\ \hline
	       CaT \textit{Stem conductance}	  & 0.05$\cdot10^{-5}$ & [0, 0.0022] & U(0,0.0001)\\ \hline
	       CaT \textit{peak}		  & 0.05$\cdot10^{-5}$ & [0, 0.0022] & U(0,0.0022)\\ \hline
	       Ka \textit{taper rate}		  & 0.1		      & ($-{\infty}$, ${\infty}$) & U(0.85,1.15)\\ \hline
	       CaT \textit{taper rate}		  & 0.1    	      & ($-{\infty}$, ${\infty}$) & U(0.85,1.15)\\ \hline
