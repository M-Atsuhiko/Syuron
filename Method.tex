...
\chapter{研究手法・原理}
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{神経細胞形態の表現}
     ...
     使用する記号の集合として$\{1,2\}$を定義する.置き換え規則として
     \begin{align*}
       rules: \hspace{0.2cm}
       & 1 \to
       \begin{cases}
       ...
       & 2 \to 2
     \end{align*}
     を定義する. 記号$1,2$を樹状突起の構造に置き換えて考えると, 記号$1$は樹状
     構造における未成長の点を表し, 記号$2$は成長済みの円柱構造を表すと考えられ
     る. また$<$\hspace{0.1cm}$>$は樹状構造の分岐を表している. すなわちこのL-Systemは
     初期状態である, ある一点から樹状構造が生成される過程を表現する. 
   ...
   \subsection{パラメータセットからのStem, Branch生成}
     ...
     上記の形態パラメータに加え樹状突起上のコンダクタンスを線形に分布させる場合は表\ref{liner_parameter}に示される
   ...
   \begin{itemize}
    ...
    \item \textbf{太さの決定}\\
      ...
      \end{tabular} \\
      ここで$diam_P$はそのBranchの親となるStemまたはBranchの太さであり, 
   ...
   \begin{enumerate}
     \item \textbf{Termination $ 1 \to 2$} \\
       %
       % Terminationのときは 1 -> 2だが、Branchは生成しなかったはず
       %
...
%
   ...
   \begin{itemize}
     \item \textbf{線形分布の場合}\\
       CaT, Kコンダクタンス値について, diameterと同様の方法で決定する\\
      \begin{tabular}[c]{ll}
        - Stemの場合: & \textit{Stem conductance} \\
        - Branchの場合: & $Conductance_{P} \times$ \textit{Conductance taper rate} \\
      \end{tabular} \\
       ここで$Conductance_{P}$はそのBranchの親のBranchやStemのコンダクタンス値である.
     \item \textbf{ガウス分布の場合}\\
...
%
 ...
 \section{シミュレーション}
   ...
   べる
   \subsection{数理モデル}
     L-systemによって作成された神経細胞形態は数理シミュレーションを行える
     よう, $5{\mu}m$ごとに1つのコンパートメントとして, その膜電位の挙動を
     ...
     \begin{itemize}
       \item CaTコンダクタンスモデル
         \begin{align}
           g_{CaT} &= \overline{g}_{CaT}m^2h  \\
           I_{CaT} &= g_{CaT}ghk(V_{i},cai,cao) \\
           KTF &= (\frac{25.0}{293.15}) \times (T + 273.15) \\
           f &= \frac{KTF}{2} \\
           nu &= \frac{V_i}{f} \\
           ghk &= -f(1.0 - \frac{C_i}{C_o}exp(nu))efun(nu) \\
           efun(z) &=
           \begin{cases}
