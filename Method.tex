...
\chapter{研究手法・原理}
 ...
 \section{神経細胞形態の作成方法}
     ...
     \begin{table}[H]
       ...
       \begin{center}
         \begin{tabular}[H]{|l|l|} \hline
           ...
           5,  \textit{Branch elevation MIEW} [$^{\circ}$]& Branchの仰角決定に用いる平均値 \\ \hline
           6,  \textit{Branch elevation SIGMA} [$^{\circ}$] & Branchの仰角決定に用いる標準偏差値 \\ \hline
           7,  \textit{Branch rotation MIEW}  [$^{\circ}$]& Branchの回転角決定に用いる平均値 \\ \hline
   ...
   \begin{itemize}
    ...
    \item \textbf{仰角の決定}\\
      Stem, の仰角はStem elevationの値である. Branchは正規分布から確率的に決定する. \\
      \begin{tabular}[c]{lll}
        - Stemの場合: & Stem elevation \\
        - Branchの場合: & $\mu = $\textit{Branch elevation MIEW} & $\sigma = $\textit{Branch elevation SIGMA} \\
      \end{tabular}
    \item \textbf{回転角の決定}\\
      仰角の場合と同様に, \\
      \begin{tabular}[c]{lll}
        - Stemの場合: & Stem rotation \\
...
%
	...
	\end{tabular}
      ...
      シミュレーションはNEURONを用いて, CVodeアルゴリズムを用いて行った. % <- NEURON シミュレータに関して書いた方がいいか？
      CVodeアルゴリズムを用いる際のエラーは$10^{-5}$未満に設定した. 

   \subsection{シミュレーションプロトコル}
     作成した神経細胞に対して以下の数理シミュレーションを行った. 
     \begin{enumerate}
       \item Blueシナプティックゾーン単独の発火
         Blueシナプティックゾーンに属するシナプスを
       \item Redシナプティックゾーン単独の発火
     \end{enumerate}
