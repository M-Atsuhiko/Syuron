   ...
   \subsection{評価値$E$の計算}
     ...
     \begin{itemize}
       ...
       \item \textbf{シミュレーションヒューリスティックス}\\
         ...
         抑制し, シミュレーションの時間中に最大の上昇幅がとれるようにするためである. 
         \begin{itemize}
   ...
   \subsection{解析項目}
     ...
     \begin{itemize}
       ...
       \item \textbf{CaTコンダクタンス含有率}
         神経細胞のCaTコンダクタンス含有率は以下のように定義する. 
         神経細胞の各コンパートメント$Comp_{i}$について, そのCaTコンダクタンスを${\overline{g}_{CaT}}_{i}$, 
         表面積を$S_{i}$とする. 神経細胞のコンパートメントの個数を$N_C$とする. 
         神経細胞のCaTコンダクタンス含有率CaT \textit{ratio}は最大CaTコンダクタンスを用いて, 
         \begin{equation}
           {\rm CaT}\hspace{1mm}ratio = \frac{{\sum}^{N_C}_{i = 1}S_{i}\cdot{\overline{g}_{CaT}}_{i}}
           {{\sum}^{N_C}_{i = 1}S_{i}\cdot Max\hspace{1mm}{\rm CaT}}
         \end{equation}
       ...
       \item \textbf{Kaコンダクタンス含有率}
         神経細胞のKaコンダクタンス含有率はCaTの場合と同様に定義する. 
