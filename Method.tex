...
%
     また, 数理モデルのパラメータを以下の表\ref{sim_param}に示す. % <- tau1 tau2は別のソースコードからとったことを書いたほうがいい
...
%           $\overline{g}_{Ka}$ [${\rm S/cm^2}$]& 0.01\\ \hline
 ...
 \section{作成した神経細胞の数理モデル化}
   ...
   \subsection{コンパートメント上のコンダクタンス分布設定}
     ...
     また, 細胞体に対してはコンダクタンスを分布させず, $\overline{g}_{{\rm Ka}} = \overline{g}_{{\rm CaT}} = 0$とした.
     ...
     \item \textbf{線形分布の場合}\\
       ...
       ${\overline{g}_{\rm CaT}}_{i,j}$, ${\overline{g}_{\rm Ka}}_{i,j}$とし,
       ...
       \item \textbf{ガウス分布の場合}\\
         ...
         またその樹状突起全体のコンパートメントでの最大のpath($Comp_{i,j}$)を
         max(Path($Comp_{i,j}$))とする.
         この二つの長さの比を$path\hspace{1mm}ratio(Comp_{i,j}) = \frac{Comp_{i,j}}{{\rm max(Path(}Comp_{i,j})}$
         とする. 
         このとき平均値$\mu$, 
       ...
       \begin{align}
         {\overline{g}_{\rm Ka}}_{i,j} &=
         \frac{{\rm G}(path\hspace{1mm}ratio(Comp_{i,j}),{\rm Ka}\hspace{1mm}Gausian\hspace{1mm}\mu,({\rm Ka}\hspace{1mm}Gausian\hspace{1mm}\sigma)^2)}
              {{\rm Ka}\hspace{1mm}Gaus\hspace{1mm}Max}
              \times {\rm Ka}\hspace{1mm}peak \\
         {\overline{g}_{\rm CaT}}_{i,j} &=
         \frac{{\rm G}(path\hspace{1mm}ratio(Comp_{i,j}),{\rm CaT}\hspace{1mm}Gausian\hspace{1mm}\mu,({\rm CaT}\hspace{1mm}Gausian\hspace{1mm}\sigma)^2)}
 ...
 \section{遺伝アルゴリズム}    % <- 「遺伝アルゴリズム」という名前はどうなのか？
   ...
   \subsection{個体評価}
     神経細胞の形態を生成した後, 以下の\ref{simulation}節に示すシミュレーションを行う.  % <- シミュレーションという言い方は良くない F値テストとか具体的に
     その結果をもとに個体の評価を行う. 個体の評価には, 個体の機能性\ref{F}のほか個体
     ...
     \begin{itemize}
       ...
       \item シミュレーションを行った個体に関しては以下の評価式によって評価値$E_{i,g}$を与える.
             ある世代$g$でシミュレーションを行った個体の添え字の集合を$S_g$とする.
             ここで個体$u_{s,g}\hspace{1mm}(s \in S_g)$とし, その中での最大の$F$値をmax$(F_g)$, また
             個体$u_{s,g}$から作られた神経細胞形態を$Neuron_{s,g}$とし, その中で最小の体積をmin$(V_{s,g})$,
             最小のコンダクタンス量をmin$(Amount Conductance)$
   ...
   \section{シミュレーション} \label{simulation}
