...
\chapter{研究手法・原理}
 ...
 \section{神経細胞の機能}
   ...
   るとする. その際に細胞体で観測される最大の脱分極量を$R$とする. Blueシナプスグループの
   ...
   input-order detectionについての概念図を以下の図\ref{input-order_detection}に示す.
     ...
     \begin{figure}[h]
       \centering
       \includegraphics[width=8cm]{./Images/TREE_sample.pdf} %シナプティックゾーンも例示する
       \caption{神経細胞の例}
       \label{Neuron}
     \end{figure}
     ...
     \begin{figure}[h]
       \centering
       \includegraphics[width=10cm]{./Images/input_order_detection.eps} 
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{樹状突起形態生成手法の基本概念}
     ...
     化しなくなるまで置き換えを行うことで得られる. また, 最も最初の記号置き換えでは必ず$1 \to 21$の
     置き換えが選択されるようにする. \\
   ...
   ひとつのBranchは長さ($length$), 太さ($diam$), 仰角($elevation$), 回転角($rotation$),
   Kaコンダクタンス値($g_{\rm Ka}$), CaTコンダクタンス($g_{\rm CaT}$), 親Branchのインデックス($parent$)を
   パラメータとして持つ. 
   よって, ある$Branch_i$は7つのパラメータからなり, 
   \begin{equation*}
    Branch_i = \{length_i, diam_i, elevation_i, rotation_i, {g_{\rm Ka}}_i, {g_{\rm CaT}}_i, parent_i\}
   \end{equation*}

   と表すことができる. またインデックス$i = 1$のとき, すなわち$Branch_1$はStemである. \\
   以下に$Branch_i$のパラメータ決定方法を述べる. 
   \begin{itemize}
    \item \textbf{長さ({$length_i$})の決定}\\
      ...
      \end{equation}
    \item \textbf{太さ($diam_i$)の決定}\\
      Branchが分岐(Bifurcation $ 1 \to 21<21>$)によって生成されるか伸張(Elongation $1 \to 21$)によって生成されるか
      によって決定方法が異なる. 
      \begin{itemize}
        \item{分岐(Bifurcation)の場合} \\
          親となる$Branch_{parent_i}$の太さ($diam_{parent_i}$)を用いて, ガウス分布($\mu = diam_{parent_i},\sigma=0.05$)
          に従う乱数を生成し, その値を$diam_i$とする. 
          \begin{equation}
            diam_i = N(diam_{parent_i},(0.05)^2)
          \end{equation}
        \item{伸張(Elongation)の場合} \\
          BranchがStemであるかどうかによって決定方法が異なり, Stemでない場合は親となる$Branch_{parent_i}$の太さ($diam_{parent_i}$)を
          実数倍した値を用いる
          \begin{equation}
            diam_i = 
            \begin{cases}
              Stem\hspace{1mm}diam & \text{(i = 1)}\\
              diam_{parent_i} \times taper\hspace{1mm}rate & \text{(otherwise)}\\
            \end{cases}\\
      \end{equation}
      ここで, $taper\hspace{0.1cm}rate = 0.875$であり, $diam$の減衰率を表す.
      \end{itemize}
    \item \textbf{仰角($elevation_i$)の決定}\\
      {\textit diam}と同様にStemであるかどうかで決定方法が異なる. 
      \begin{equation}
        elevation_i = 
        \begin{cases}
   ...
   \item \textbf{コンダクタンス値(${g_{\rm CaT}}_i$, ${g_{\rm Ka}}_i$)の決定} \\
     コンダクタンス値は, 樹状突起上のコンダクタンス分布を線形分布とするかガウス分布とするかで決定方法が異なる. 
     \begin{itemize}
       \item \textbf{線形分布の場合}\\
         ${g_{\rm CaT}}_i$, ${g_{\rm Ka}}_i$について, 上記の$diam$と同様の方法で決定する\\
         \begin{align}
           {g_{\rm CaT}}_i &= 
           \begin{cases}
...
%
  ...
  \subsubsection{記号1の置き換え規則($*$)}
    形態生成L-systemにおける記号$2$はBranchに対応している.
    初期状態の記号$1$の座標を原点$(0,0,0)$とすると, 全ての記号$1$には3次元空間上の
    座標を設定することができる. 原点から, ある記号$1_i$に至
    るまでに通過するStemやBranchの長さの和を${\rm path(1_i)}$とする. ここで記号
    $1$の置き換え規則($*$)の選択方法を以下に示す.
    \begin{enumerate}
      \item \textbf{停止 Termination $ 1 \to 3$} \\
        $[0,1]$の範囲で一様分布に従う乱数を一つ生成し, その値を$rand$とする.
        累積ガンマ分布\textit{Cumulative} $\gamma$(x)を用いて記号$1_i$の伸張停止
      ...
      \item \textbf{分岐 Bifurcation $ 1 \to 21<21>$} \\
      ...
      \item \textbf{伸張 Elongation $1 \to 21$} \\
        上記の条件を満たさなかった場合, $1 \to 21$とする. 
    \end{enumerate}

  \subsubsection{形態的ヒューリスティック}
    ただし, 以下の条件に該当する場合は強制的に伸張停止とする. これは樹
    状突起として妥当な形態を得るために導入するヒューリスティックスである.    % <- 妥当とは?
    \begin{itemize}
      \item ${\rm path(1_i)} > 2000$のとき
      \item $diam_{parent_i} <= Minimum diameter$ のとき\\ % <- パラメータをあとでまとめたほうがいいか？
       また, この場合$Branch_{parent_i}$の$diam_{parent_i}$をMinimum diameterに設定し直す. 
     \end{itemize}
