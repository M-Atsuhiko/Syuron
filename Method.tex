...
\chapter{研究手法・原理}
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{樹状突起形態生成手法の基本概念}
     ...
     \begin{align*}
       ...
       & B \to B \\
       & T \to T
     \end{align*} % <- これの置き換え結果を例示してみる

     を定義する. 記号$S,B,T$を樹状突起の構造に置き換えると, 記号Sは樹状
     構造における未成長の点を表し, 記号BはBranch, 記号Tは樹状突起の終端点に対応する.
     ここで, $<$\hspace{0.1cm}$>$は樹状構造の分岐を表している. よって, 定義したL-Systemは
     1つの未成長点を初期状態とし, そこから樹状構造が生成されていく過程を表現している. 
     記号Sの置き換え規則($*$)は決定的ではなく, 後に述べる分岐確率や終端確率によって決定
     される. 1つの樹状突起は初期状態$\{S\}$から, このL-Systemを開始し記号列が変
     化しなくなるまで置き換えを行うことで得られる. また, 最も最初の記号置き換えでは必ず$S \to BS$の
     置き換えが選択されるようにする. \\
     最終的に得られる記号列は記号$B$と記号$T$からなり, 最も左端にある記号$B$がStemを表す. 
 ...
 \subsection{記号Sの置き換え規則($*$)}
   形態生成L-systemにおける記号$B$はBranchに対応し, 文字列は樹状構造を表現する. 
   文字列中のある記号$S$に至るまでに通過する記号$B$をBranchに置き換え, その$length$の和をとり, % <- これも図をはるか?
   それを${\rm path(S)}$とする. 
   \begin{figure}[H]
     \centering
     ...
     \label{path(S)}
   \end{figure}
   以下に記号$S$の置き換え規則($*$)の選択方法と, その際に考慮するヒューリスティックスについて示す. 
   \begin{enumerate}
     \item \textbf{停止 Termination $S \to T$} \\
        $[0,1]$の範囲で一様分布に従う乱数を一つ生成し, その値を$rand$とする.
        累積ガンマ分布\textit{Cumulative} $\gamma$(x)を用いて記号$S$の伸張停止
      ...
      \item \textbf{分岐 Bifurcation $ S \to BS<BS>$} \\
	  ...
	  Scaled\hspace{0.1cm}\gamma\hspace{0.05cm}(path(S)) > rand
        \end{equation}
        のとき$ S \to BS<BS>$とする. すなわちmakeBranch($diam_i$)より, 新たな$Branch$を2つ生成する. 
      \item \textbf{伸張 Elongation $S \to BS$} \\
        上記の条件を満たさなかった場合, $S \to BS$とする. 
        すなわちmakeBranch($diam_i$)より, 新たな$Branch$を1つ生成する. 
    \end{enumerate}
   ...
   \subsubsection{形態的ヒューリスティック}
    ...
    \begin{itemize}
      ...
      \item $diam_{parent} \le Minimum\hspace{1mm}diameter$ のとき\\ % <- パラメータをあとでまとめたほうがいいか？
       また, この場合, 親$Branch$の$diam$を$Minimum\hspace{1mm}diameter$に設定し直す. 
    \end{itemize}
   ...
   \end{itemize}
    ...
    \subsection{シナプティックゾーン}
     ...
     場合, $Branch$上にシナプスを付与する. 具体的には下記\ref{synapse_setting}にて詳述する. 

  \section{使用する数理モデル}
    \subsection{神経細胞モデル, シナプスモデル}
     L-systemによって作成された神経細胞はマルチコンパートモデルによって  % Somaのこと書くの忘れてた
     その膜電位の挙動をシミュレーションする. 
     ...
     また, コンパートメントに他のコンパートメントが結合している場合は以下の式\ref{next_comp}を式\ref{V_i}に追加する. 
   ...
   \subsection{イオンチャネルモデル}
     また, 式\ref{V_i}の$I_{CaT}$, $I_{Ka}$はMigliore et alに示されるイオンチャネルの
...
%
 ...
 \section{作成した神経細胞の数理モデル化}
   L-Systemによって得られた神経細胞を前節で示した数理モデルに変換する手法を示す. 
   ...
   $Comp_{i,j}$のそれぞれのコンパートメントについて式\ref{V_i}を用いて膜電位を計算する. 

   %
   %
   % このへんで図をいれるとわかりやすいかもしれない
   %
   ...
   \subsection{コンパートメント上のコンダクタンス分布設定}
     ...
     コンダクタンス値の決定方法は線形分布を用いるかガウス分布を用いるかによって異なる. \\
     \begin{itemize}
     \item \textbf{線形分布の場合}\\
       ...
       \item \textbf{ガウス分布の場合}\\
         コンパートメント単位でコンダクタンス値を設定する. Stemから$Comp_{i,j}$までの$length$の合計をpath($Comp_{i,j}$)とし, 
         またその樹状突起全体のコンパートメントでの最大のpath($Comp_{i,j}$)をmax(Path($Comp_{i,j}$))とする. このとき平均値$\mu$, 
         標準偏差$\sigma$のガウス分布から実数$x$に対する確率密度を計算する関数G($x,\mu,{\sigma}^2$)について
         \begin{align}
   ...
   \begin{figure}[H]
     \centering
     \includegraphics[width=5cm]{./Images/temp.png} %図で説明する
     \caption{${\rm path(S)}$}
     \label{path(S)}
   \end{figure}
       \end{itemize}
