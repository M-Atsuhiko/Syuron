...
%           $\overline{g}_{Ka}$ [${\rm S/cm^2}$]& 0.01\\ \hline
 ...
 \section{作成した神経細胞の数理モデル化}
   ...
   電位を計算する.\\
   また, $Soma_i$に対しても同様に式\ref{V_i}を用いて膜電位の挙動をシミュレー
   トする. ただし$Soma$に対してはコンパートメントへの分割は行わず, 1つの式\ref{V_i}
   のみによって膜電位変動をシミュレートする. 
   \subsection{コンパートメント上のコンダクタンス分布設定}
     ...
     また, $Soma$に対してはコンダクタンスを分布させず, $\overline{g_{soma}}_{{\rm Ka}} = \overline{g_{soma}}_{{\rm CaT}} = 0$とした.
 ...
 \section{遺伝アルゴリズム}    % <- 「遺伝アルゴリズム」という名前はどうなのか？
   ...
   また, ある世代数$g$での$i$番目の個体を$u_{g,i}$と表すとする.
   この遺伝アルゴリズムにおける遺伝子は個体$u_{g,i}$の持つパラメータセット$P_{g,i,1},P_{g,i,2}$である. 
   ...
   \subsection{個体評価}
     ...
     \subsubsection{シミュレーションプロトコル}
       ...
       またBlueシナプスグループに属するシナプスを活性化させる時刻を$t_{Blue}$[ms],
       Redシナプスグループに属すシナプスを活性化させる時刻を$t_{Red}$[ms]とする.
       シミュレーション時間はCaTコンダクタンスが単独で分布している場合は100ms, それ以外の場合
       は50msである. また, $Neuron{g,i}$の細胞体$Soma_{g,i}$で観測される最大の膜電位上昇量を
   ...
   \subsection{評価値$E$の計算}
     ...
     \begin{itemize}
       \item \textbf{形態的エラー}\\
         ...
         \end{equation}
         とする. 
       \item シミュレーションプロトコルを実行した個体に対しては, その世代での相対的な
         評価値を与える. シミュレーションプロトコルを実行することができた個体の
         添え字の集合を$S_g$とする.
         ここで個体$u_{g,s}\hspace{1mm}(s \in S_g)$について, 
         その中での最大の$F$値をmax$(F_g)$, また個体$u_{g,s}$から作られた
         神経細胞形態を$Neuron_{g,s}$とし, その中で最小の体積をmin$(Vol_{g,s})$,
         ...
         \begin{align}
           Function Minus_{g,i} &= \frac{F_{g,i}}{F_{max,g}} \\
           Morphology Minus_{g,i} &= \frac{Volume_{g,i}}{Volume_{max,g}} \\
           Conductance Minus_{g,i} &= \frac{Conductance ratio_{g,i}}{Conductance_{max,g}} \\ %あとで詳しく書く
         \end{align}
     ...
     \begin{itemize}
       \item \textbf{形態的ヒューリスティックス}\\
         以下の場合はシミュレーションを行わず, 式\ref{Morpho_E}によって評価値$E_{g,i}$を与えた. 
         \begin{itemize}
       ...
       \item \textbf{シミュレーションヒューリスティックス}\\
         ...
         抑制するためである.
         \begin{itemize}
           \item $ 2 > {R_{Blue}}_i$または$ 2 > {R_{Red}}_i$のとき
           \item ${R_{Blue}}_i > 25$または${R_{Red}}_i > 25$のとき
	   \item シミュレーションの最後の5ステップで膜電位の上昇が起こったとき
         \end{itemize}
         この条件に当てはまったとき, ヒューリスティックスとして
         \begin{equation}
           E_{g,i} = N(-50,5^2) % <- この書き方はあってるのか
           \label{penalty}
         \end{equation}
         とする. 
     \end{itemize}
   ...
   %交叉、突然変異を起こす個体の数は中心極限定理的な考えで求められる
     ...
     る. 一点交叉は評価値上位$\mu$個の個体($u_{g,r_{1}},u_{g,r_{2}},\dots,u_{g,r_{\mu}}$)を親として, $\lambda$個の子個体
	...
	\dots \\
	{\rm Cross\hspace{1mm}over(Parent_g)}_{g + 1,1 + \lambda}, \\ % <- これはわかりにくい
	{\rm Mutation(u_{g,r_{1}})}, \\
	\dots \\
	{\rm Mutation(u_{r_{g,199 - \lambda}})}
       \end{array}
       \right)
     \end{equation}
 ...
 \section{実験手順}
   ...
   \subsection{実験における独立変数}
     ...
     2つのシナプスグループの活性化時間の差${\Delta}t$, 樹状突起上のコンダクタンス分布様式
     である. それぞれの変数について総当たりで遺伝アルゴリズムを用い, 最適な$Neuron$を求めた.
     また, いずれの試行においても試行数は10回である. 
     \begin{enumerate}
       \item \textbf{Ka, CaTイオンチャネルの有無} \\
         本実験では神経細胞のイオンチャネルとしてKaチャネル, CaTチャネルを用いるが,
         それぞれを用いるパターンと用いないパターンを用意した. よってパターンとしては
         表\ref{ion-channel_pattern}の4つがある. なお, チャネルを用いないパターン
