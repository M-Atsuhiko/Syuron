...
\chapter{研究手法・原理}
 ...
 \section{神経細胞の機能}
   ...
   その際に細胞体で観測される最大の膜電位上昇量を$R$とする. Blueシナプティックゾーンの
   シナプスが先に活性化する場合の$R$を$R_{B{\to}R}$とし, Redシナプティックゾーンのシナプスが
...
%  Passiveモデルを用いると, 膜特性が膜電位によって変化しないため細胞体での膜電位上昇は
 ...
 \section{神経細胞形態の作成方法}
   ...
   \subsection{樹状突起形態生成手法の基本概念}
     ...
     \end{align*}
     樹状突起形態の生成方法として, 三宮 et al. を参考にすると以下のようなL-systemが考えられる\cite{三宮信夫1998遺伝アルゴリズムと最適化}. 
   ...
   \subsection{パラメータセット, $Branch$, $Dendrite$, $Neuron$の定義}
     \subsubsection{パラメータセットの定義}
       ...
       \begin{table}[H]
         ...
         \begin{center}
           \begin{tabular}[H]{|c|l|l|} \hline
             \multicolumn{1}{|c}{No.} & \multicolumn{1}{|c}{パラメータ名} & \multicolumn{1}{|c|}{説明} \\ \hline \hline
             ...
             4 &  \textit{Stem rotation}  [$^{\circ}$]& Stemの回転角 \\ \hline
       ...
       指数関数的な分布, またはガウス分布のどちらかの分布様式を用いる. 以下ではコンダクタンスを指数関数的に
       分布させる分布様式を指数関数分布と呼ぶ. 一般的な指数分布とは異なるものであることに注意されたい. 
       樹状突起上のコンダクタンスを指数関数分布させる場合は
       表\ref{exp_parameter}に示される指数関数分布コンダクタンスパラメータを,
       ガウス分布に従って分布させる場合には
       表\ref{gausian_parameter}に示されるガウス分布コンダクタンスパラメータをそれぞれ用いる.
       

       \begin{table}[H]
         \caption{指数関数分布コンダクタンスパラメータ}
         \label{exp_parameter}
         \begin{center}
           \begin{tabular}[H]{|c|l|l|} \hline
             \multicolumn{1}{|c}{No.} & \multicolumn{1}{|c}{パラメータ名} & \multicolumn{1}{|c|}{説明} \\ \hline \hline
 ...
 \subsection{記号Sの置き換え規則}
   ...
   \begin{enumerate}
     \item \textbf{停止 Termination $S_i \to T_{N_T + 1}$} \\
      ...
      \item \textbf{分岐 Bifurcation $ S_i \to <B_{N_B + 1}S_{N_B + 1}><B_{N_B + 2}S_{N_B + 2}>$} \\
        ...
        Scaled $\gamma$($x,k,\theta$)を用を用いて分岐の判定を行う. 
   ...
   \subsubsection{形態的ヒューリスティック}
    ...
    \end{itemize}
    これらのヒューリスティックは神経細胞形態を作成するために使用されることが多い\cite{burke1992parsimonious}. 
...
%           $\overline{g}_{Ka}$ [${\rm S/cm^2}$]& 0.01\\ \hline
   ...
   \subsection{コンパートメント上のコンダクタンス分布設定}
     ...
     を設定する. 
     樹状突起上のコンダクタンス分布は指数関数分布とガウス分布のどちらかを使用する.
     指数関数分布は各$Branch$のコンダクタンスの値を, 親$Branch$のコンダクタンス値の実数倍として
     設定する\cite{torben2009systematic}. 
     ガウス分布は$Comp$ごとにコンダクタンスを変化させる.  よって指数関数分布の場合よりも細かくコンダクタンス
     分布を設定できる. また, 樹状突起のある一部のみに高いコンダクタンス分布を持たせることも可能である. 
     $Soma$に対してはコンダクタンスを分布させず,
     ${\overline{g}_{soma}}_{{\rm Ka}} = {{\overline{g}}_{soma}}_{\rm CaT} = 0$とした.
     また, コンダクタンスの取りうるCaTコンダクタンスの最大値を\textit{Max} CaT, Kaの最大値を\textit{Max} Kaとし,
     以下の表に示す.
     ...
     \begin{itemize}
     \item \textbf{指数関数分布の場合}\\
       $Branch$単位でコンダクタンスを設定する. つまり, ある$Branch_i$から分割されたコンパートメント$Comp_{i,j}$は
       ...
       \item \textbf{ガウス分布の場合}\\
         ...
         この二つの長さの比を\\
         path\hspace{1mm}ratio($Comp_{i,j}$)とし,
         \begin{equation}
 ...
 \section{遺伝アルゴリズム}    % <- 「遺伝アルゴリズム」という名前はどうなのか？
   ...
   である. また, 世代数$g = 1$における個体のパラメータの初期値は$P_{1,i,1},P_{1,i,2}$ともに
   以下の表\ref{sigma_M_init}に示す値を用いる. 表\ref{sigma_M_init}中の記号
   U($\alpha,\beta$)は$[\alpha,\beta]$から一様乱数を1つ与える関数であるとする. 
   ...
   \subsection{個体評価}
     ...
     \subsubsection{シミュレーションプロトコル}
       ...
       シミュレーション時間は50msである. また, $Neuron_{g,i}$の細胞体$Soma_{g,i}$で観測される最大の膜電位上昇量を
       $R$とする. シミュレーションは以下の表\ref{synapse_time}に示される$t_{Blue}$, $t_{Red}$の組みを用いて,
   ...
   \subsection{評価値$E$の計算}
     ...
     \begin{itemize}
       ...
       \item 個体から生成した$Neuron$が両方のシナプティックゾーンにシナプスを持ち, 上記4つの
         ...
         をかけた値の総量である. このとき, ある個体$u_{g,i}$の評価値$E_{g,i}$は以下のように決定する.
     ...
     \begin{itemize}
       ...
       \item \textbf{シミュレーションヒューリスティックス}\\
         ...
         その値を評価値$E_{i,g}$とした.
         これは神経細胞での発火や, 膜電位の振動を抑制し, シミュレーションの時間中に最大の上昇幅がとれるようにするためである. 
         \begin{itemize}
	   ...
	   \item シミュレーションの最後の5ステップで膜電位の上昇が起こったとき
         ...
         \begin{equation}
           E_{g,i} = {\rm G}(-50,5^2) % <- この書き方はあってるのか
           \label{penalty}
         \end{equation}
         とする. 
     \end{itemize}
	...
	{\rm Mutation(u_{r_{g,199 - \lambda}})_{g + 1,200}}
     ...
     \begin{itemize}
       ...
       \item \textbf{突然変異 Mutation}\\
        パラメータセットの各項目に対して0.65の確率でガウス分布G($\mu$,${{\sigma}_M}^2$)に従う変動を与える. すなわち, あるパラメータ値
         ...
         ${p'}_i$を取り直す. 
         \begin{table}[H]
           \caption{突然変異に用いる標準偏差${\sigma}_M$と各パラメータの初期値}
           \label{sigma_M_init}
           \begin{center}
	     \begin{tabular}[t]{|l||c|c|c|c|} \hline
           \multicolumn{1}{|c}{パラメータ名} & \multicolumn{1}{|c|}{${\sigma}_M$} & \multicolumn{1}{c|}{値域} & \multicolumn{1}{c|}{初期値}\\ \hline \hline
 ...
 \section{実験手順}
   ...
   \subsection{実験における独立変数}
     ...
     個体評価(式\ref{E_equation})におけるコンダクタンスへの考慮の有無での4種類である. 
     それぞれの変数の総当たりの組に対して上記の遺伝アルゴリズムを用い, 最適な$Neuron$を求めた.
     ...
     \begin{enumerate}
       ...
       \item \textbf{個体評価におけるコンダクタンス量の評価}
         個体評価にコンダクタンスの量を考慮する場合と, しない場合で式\ref{E_equation}の
         $F$値, 細胞形態, コンダクタンス量に対する重みの値に以下の表\ref{E_ratio}の値を
         用いた.
         \begin{table}[H]
           \caption{個体評価に使用する重み}
           \label{E_ratio}
           \begin{center}
             \begin{tabular}[h]{|c||c|c|c|} \hline
               ...
               1, コンダクタンスの考慮なし & 75 & 25 & 0\\ \hline
               2, コンダクタンスの考慮あり & 75 & 20 & 5\\ \hline
             \end{tabular}
           \end{center}
         \end{table}
         コンダクタンス考慮なしの重みの組は, Passiveタイプの神経細胞のシミュレーションにて先行研究の結果を再現することが
         できた値の組である. {\textit Morpho ratio}と{\rm Conductance ratio}は大きな値をとるほど, $Neuron$の大きさと
         コンダクタンス分布の自由度を低下させると考えられ, input-order detectionを実現する上で負の効果を持つと
         考えられる. よってコンダクタンスの考慮ありの場合の{\textit Function ratio}と,
         {\textit Morpho ratio}と{\rm Conductance ratio}の和の比はコンダクタンス考慮なしの場合と等しくなるようにした. 
     \end{enumerate}     
   \subsection{解析項目}
     実験によって得られた最終世代における最良個体の形態, コンダクタンス分布について, 以下の項目について解析を行った. 
     \begin{itemize}
       \item \textbf{機能性$F$の値}
       \item \textbf{長さ[${\mu}m$]}\\
         神経細胞の持つ全ての$Branch$の$length$の和
       \item \textbf{体積[${\mu}m^3$]} \\
         神経細胞の全ての$Branch$の体積の和

       \item \textbf{樹状突起の直径} \\
         本研究の形態生成では, 樹状突起の$Branch$の太さはStemの値をもとに決定される. そのためStemの直径を
         その樹状突起の代表的な太さとして用いた. 

       \item \textbf{樹状突起の分岐数} \\
         樹状突起に含まれる分岐の個数の合計

       \item \textbf{樹状突起のシナプス数} \\
         樹状突起がもつシナプスの個数の合計

       \item \textbf{Kaコンダクタンス含有率} \\
         神経細胞のKaコンダクタンス含有率は以下のように定義する. 
         神経細胞の各コンパートメント$Comp_{i}$について, そのKaコンダクタンスを${\overline{g}_{Ka}}_{i}$, 
         表面積を$S_{i}$とする. 神経細胞のコンパートメントの個数を$N_C$とする. 
         神経細胞のKaコンダクタンス含有率Ka \textit{ratio}は, 最大Kaコンダクタンス量である
         Max Kaを用いて
         \begin{equation}
       ...
       \item \textbf{CaTコンダクタンス含有率} \\
         神経細胞のCaTコンダクタンス含有率は, Kaコンダクタンスの場合と同様に定義する. 

        \item \textbf{ガウス分布させたコンダクタンスのピーク位置} \\
          コンダクタンスをガウス分布に従って分布させた場合のピークの位置($Gausian\hspace{1mm}\mu$)を, 
          樹状突起の長さを[0,1]に正規化した値で表す. 値が1に近いほど樹状突起の先端にピーク位置があることを表す. 
          しかし$peak$が0に近い場合は$\mu$, $\sigma$の値は意味をなさない. そのためピーク位置の解析では
          最大コンダクタンス値の5\%以上の$peak$をとった場合の$Gausian\hspace{1mm}\mu$のみを用いた.
     \end{itemize}
