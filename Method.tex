...
%
 ...
 \section{遺伝アルゴリズム}    % <- 「遺伝アルゴリズム」という名前はどうなのか？
   ...
   また$\overrightarrow{P_i}$から生成される樹状突起を${\rm Dend_i}$とする. よって1つの個体$u_i$から生成される
   神経細胞は$Neuron_i = \{Dend_{i,1},Dend_{i,2}\}$
   この遺伝アルゴリズムにおける遺伝子とは個体の持つパラメータセットとなる. 
   ...
   \subsection{個体評価}
     ...
     \begin{itemize}
       \item \textbf{形態的エラー}\\
         ...
         $Dend_1$の$y$座標の最大値をmax($y_A$), 樹状突起Bの$y$座標の最小値をmin($y_B$)とし % <- このへんも書き直す
         評価値$E$を
         \begin{equation}
   ...
   \subsection{エリート保存}
     各世代において最大の評価値をとった個体のパラメータセットはそのまま次世代の1個体に
     引き継がれる. 
   ...
   %交叉、突然変異を起こす個体の数は中心極限定理的な考えで求められる
     ...
     \begin{itemize}
       \item \textbf{一点交叉 Cross over}\\
         評価値上位の$\mu$個体から確率的に親を2個体選択する. 評価値がi番目の % <- 評価値が負の個体なども存在し、ルーレット選択などが使えないということも書く
         個体が選択される可能性を$P_i$とすると,
         \begin{equation} % 選択確率を円グラフで図示するか？
	   P_i =  
	   \begin{cases}
	     (\frac{1}{2})^i &  i < \mu\\
	     (\frac{1}{2})^{i - 1} & otherwise
	   \end{cases}\\
         \end{equation}
